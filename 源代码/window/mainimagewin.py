...
# Form implementation generated from reading ui file 'mainimagewin.ui'
#
...
"border-color: rgb(242, 216, 121);")
        ...
        self.btnOpenCam = QtWidgets.QPushButton(Dialog)
        self.btnOpenCam.setGeometry(QtCore.QRect(70, 70, 100, 35))
        self.btnOpenCam.setStyleSheet("font: 14pt \"汉仪喵魂体W\";\n"
"color: rgb(0, 0, 0);\n"
"border:2px solid;\n"
"border-color: rgb(255, 255, 255);\n"
"background-color: rgb(242, 216, 121);")
        ...
        self.btnStopGrab = QtWidgets.QPushButton(Dialog)
        self.btnStopGrab.setGeometry(QtCore.QRect(350, 70, 100, 35))
        self.btnStopGrab.setStyleSheet("font: 14pt \"汉仪喵魂体W\";\n"
"color: rgb(0, 0, 0);\n"
"border:2px solid;\n"
"border-color: rgb(255, 255, 255);\n"
"background-color: rgb(242, 216, 121);")
...
"background-color: rgb(242, 216, 121);")
        ...
        self.btnStartGrab = QtWidgets.QPushButton(Dialog)
        self.btnStartGrab.setGeometry(QtCore.QRect(210, 70, 100, 35))
        self.btnStartGrab.setStyleSheet("font: 14pt \"汉仪喵魂体W\";\n"
"color: rgb(0, 0, 0);\n"
"border:2px solid;\n"
"border-color: rgb(255, 255, 255);\n"
"background-color: rgb(242, 216, 121);")
    ...
    def retranslateUi(self, Dialog):
        ...
        Dialog.setWindowTitle(_translate("Dialog", "图像采集"))
        self.btnCloseCam.setText(_translate("Dialog", "关闭相机"))
        self.btnSaveImage.setText(_translate("Dialog", "保存图像"))
        self.LLabel.setText(_translate("Dialog", "左图像区域"))
        self.btnOpenCam.setText(_translate("Dialog", "启动相机"))
        self.btnStopGrab.setText(_translate("Dialog", "暂停采集"))
        self.RLabel.setText(_translate("Dialog", "右图像区域"))
        self.ResultLabel.setText(_translate("Dialog", "结果显示区域"))
        self.btnStartGrab.setText(_translate("Dialog", "采集图像"))
    ...
    def saveImage(self):
        ...
        self.ResultLabel.setText("左图像保存正常！")

        # 保存右图像
        idn = MVGetSampleGrab(self.hCam2, self.himage2)
        fname = QFileDialog.getSaveFileName(
            self, '打开文件', './Images' + str(idn.idn) + '.bmp', ("Images (*.bmp *.jpg *.tif *.raw)"))
        filename2 = os.path.basename(fname[0])  # 获取到需要存储的文件名
        pathname2 = os.path.join(os.getcwd(), filename2)  # 获取带有文件名的文件路径
        newfile2 = '\\'.join(fname[0].split('/')[:-1])  # 需要存储到的新文件夹
        MVImageSave(self.himage2, filename2.encode('utf-8'))  # 将图片保存下来
        if (newfile != os.getcwd()):  # 将图片移动到指定文件夹下
            try:
                shutil.move(pathname2, newfile)
            except:
                os.unlink(os.path.join(newfile, filename2))
                shutil.move(pathname2, newfile)
        image2 = QImage(newfile2 + '\\' + filename2)
        self.RLabel.setPixmap(QPixmap.fromImage(image2))  # 加载图片
        self.ResultLabel.setText( "右图像保存正常！")

        self.ResultLabel.setText( "图像保存正常！\n"
                                  "左图像保存路径：" + str(pathname) + "\n"
                                  "右图像保存路径：" + str(pathname2))
...
class MyWindow(QtWidgets.QMainWindow, Ui_Dialog_image):
    def __init__(self):
